...
unsigned long motor_check_interval = 100;  
...
void setup() {
  ...
  Serial1.begin(115200);  // XV LDS data (connects to PA10 on Blue Pill)
...
void loop() {
  ...
  if (xv_config.motor_enable) {  
    if (rpmPID.Compute()) {
        toggleLED();
        int pwm_val_int = int(0.5 + pwm_val);
        analogWrite(motorPin, pwm_val_int); 
      }
    }
...
void initEEPROM() {
  ...
  strcpy(xv_config.version, "1.3.1");
  //xv_config.motor_pwm_pin = motorPin;  // pin connected N-Channel Mosfet

  xv_config.rpm_setpoint = 300.0;  // desired RPM
  xv_config.rpm_min = 200.0;
  xv_config.rpm_max = 400.0;
  xv_config.pwm_min =   1.0;
  xv_config.pwm_max = 255.0;
  xv_config.sample_time = 200;
  xv_config.Kp = 1.50;
  xv_config.Ki = 1.10;
  xv_config.Kd = 0.5;
...
void motorOn() {
  xv_config.motor_enable = true;
  analogWrite(motorPin, int(0.5 + pwm_val));
